...
const baseStep = {
	...
	_prepareProperties(manager, scopeReporter, name, stepConfiguration, endpoints) {
		...
		ATTRIBUTES.forEach(a => {
			//console.log(`${a}: ${stepConfiguration[a]} : ${this[a]}`);
			if (stepConfiguration[a] !== undefined /*&& stepConfiguration[a] !== this[a]*/ ) {
				props[a] = {
	...
	finalize(manager, scopeReporter, stepConfiguration) {
		this.createPredefinedEndpoints(scopeReporter, stepConfiguration);
	},

	getInstance(manager, scopeReporter, stepDefinition) {
		const newInstance = support.create(manager, scopeReporter, this, stepDefinition, stepDefinition.name);
	...
	_createEndpoints(scopeReporter, impl, def) {
		...
		// LINK: This does not work. After the change like you could see in the next lines it works.
		// const allEndpointNames = new Set(impl.endpoints ? Object.keys(impl.endpoints) :
		// 	undefined);
		const allEndpointNames = new Set();

		// Add the endpoint of the step implementation.
		if (impl.endpoints) {
			for (const endpointName in impl.endpoints) {
				allEndpointNames.add(endpointName);
			}
		}
