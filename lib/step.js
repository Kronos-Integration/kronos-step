...
const baseStep = {
	 ...
	 * @api protected
	 */
	_create(manager, scopeReporter, name, stepConfiguration) {
		...
		const endpoints = this._createEndpoints(scopeReporter, stepConfiguration);

		// prepare object properties
		const props = this._prepareProperties(manager, scopeReporter, name, stepConfiguration, endpoints);

		let newStep = this._initialize(manager, scopeReporter, name, stepConfiguration, endpoints, props);

		newStep = Object.create(newStep, props);
	 ...
	 * @api protected
	 */
	_createPredefinedEndpoints(scopeReporter, endpoints, stepConfiguration) {
	...
	_createEndpoints(scopeReporter, def, impl) {
		...
		const allEndpointNames = new Set(this.endpoints ? Object.keys(this.endpoints) :
			undefined);
		...
		for (const en of allEndpointNames) {
			...
			const ie = this.endpoints && this.endpoints[en] ? this.endpoints[
				en] : undefined;
		...
		this._createPredefinedEndpoints(scopeReporter, endpoints, def);
