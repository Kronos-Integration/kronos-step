...
const baseStep = {
	...
	_create(manager, scopeReporter, name, stepConfiguration) {
		...
		console.log(`THIS: ${this.name} ${parent}`);
		// create the endpoints from what we know

		const endpoints = parent._createEndpoints.call(this, scopeReporter, stepConfiguration, this);

		// prepare object properties
		const props = parent._prepareProperties.call(this, manager, scopeReporter, name, stepConfiguration, endpoints);

		let newStep = parent._initialize(manager, scopeReporter, name, stepConfiguration, endpoints, props);
	...
	_createPredefinedEndpoints(scopeReporter, stepConfiguration) {
		...
		this.endpoints.log = endpoint.createEndpoint("log", {}, logEndpoint);
