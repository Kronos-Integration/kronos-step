	...
	createEndpoint(scopeReporter, name, def) {
		...
		if (def.interceptors) {
			ep.interceptors = def.interceptors.map(icDef => {
				const InterceptorClass = this.manager.interceptors[icDef.type];

				//console.log(`INTERCEPTOR: ${icDef.type} -> ${InterceptorClass}`);

				if (InterceptorClass) {
					return new InterceptorClass(ep, icDef);
				} else {
					scopeReporter.exception(`The interceptor '${icDef.type}' could not be found in the manager'`);
				}
			});
		}
