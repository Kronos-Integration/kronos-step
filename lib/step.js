...
const BaseStep = {
	...
	createEndpoint(name, def) {
		...
		if (def.in) ep = new endpoint.ReceiveEndpoint(name, this, this.endpointOptions(name, def));
		if (def.out) ep = new endpoint.SendEndpoint(name, this, this.endpointOptions(name, def));
