	...
	createEndpoints(manager, scopeReporter, impl, def) {
		...
		for (const endpointName in impl.endpoints) {
			...
			if (endpointConfiguration) {
				endpoints[endpointName] = endpoint.createEndpoint(endpointName, endpointConfiguration,
					endpointImplementationDefinition);
			} else {
				endpoints[endpointName] = endpoint.createEndpoint(endpointName, endpointImplementationDefinition);
