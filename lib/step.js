...
const BaseStep = {
	...
	createEndpoints(scopeReporter, impl, def) {
		if (impl.endpoints) {
			Object.keys(impl.endpoints).forEach(name => {
				this.createEndpoint(scopeReporter, name, impl.endpoints[name], def.endpoints[name]);
			});
		}

		if (def && def.endpoints) {
			Object.keys(def.endpoints).forEach(name => {
				if (impl.endpoints[name]) return;
				this.createEndpoint(scopeReporter, name, impl.endpoints[name], def.endpoints[name]);
			});
		}
	...
	createEndpoint(scopeReporter, name, impl, def) {
		if (d.in) this.addEndpoint(new endpoint.ReceiveEndpoint(name, this));
		if (d.out) this.addEndpoint(new endpoint.SendEndpoint(name, this));
	},
