...
class Endpoint {
	...
	connect(otherEndpoint) {
		...
		if (self.isConnected() && self.isConnected() === otherEndpoint) {
			// nothing to do, already connected with the given endpoint
		} else {
			if (self.isConnected()) {
				throw new Error(
					`Could not connect the endpoint, the endpoint '${self.name}' is already connected with '${self.connectedEndpoint.name}'`
				);
			}
			...
			if (self.in && otherEndpoint.out) {
				if (self.passive && otherEndpoint.active) {
					otherEndpoint._connectInPassive(self);
				} else if (self.active && otherEndpoint.passive) {
					// not yet supported
					throw new Error(
						`Could not connect the endpoint '${self.name}' with the endpoint '${otherEndpoint.name}'. The combination 'out and passive' with 'in and active' is currently not supported`
					);
				}
				...
				if (self.passive && otherEndpoint.active) {
					...
					self._connectInPassive(otherEndpoint);
				}
			}

			if (!self.isConnected()) {
				throw new Error(`Could not connect the endpoint '${self.name}' with the endpoint '${otherEndpoint.name}'`);
			}
		}
	}
}
...
module.exports.createEndpoint = function (endpointName, endpointConfiguration, metaDefinition) {
	return new Endpoint(endpointName, endpointConfiguration);
};
