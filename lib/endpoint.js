...
class Endpoint {
	...
	toString() {
		...
		return `${this.name}:${['in','out','active','passive'].filter( a => this[a]).join(':')}`;
	}
	...
	setPassiveGenerator(generator) {
		if (this.in && this.passive) {
			...
			throw new Error(`${this}: It is not possible to set a genertaor if an endpoint is not 'in' and not 'passive'`);
		}
	}
	...
	connect(otherEndpoint) {
		if (!otherEndpoint) {
			throw new Error(`${this}: No endpoint given to be connected`);
		}
